...
# get_dists_vec
# 'targets' and 'neighbours' can have common elements
# 'neighbours' should contain all observations

get_dist_vec <- function(targets, neighbours){
  targets <- data.matrix(targets)
  neighbours <- data.matrix(neighbours)
  return(distm(neighbours[,c("lng","lat")],targets[,c("lng","lat")]))
}

# test <- get_dist_vec(source[1:3,c("lat","lng")],source[,c("lat","lng")])

# get_each_nc
# Calculate the neighbourhood components for observation i

get_each_nc <- function(targets,neighbours,cat_ids,cats_unique){
  ...
  cats_matrix[nrow(neighbours),] <- rep(FALSE,n_cats)    # Exclude self

  # Transformation

  dist_vec <- get_dist_vec(targets[,c("lat","lng")],neighbours[,c("lat","lng")])
...
targets <- source[1:3,c("lat","lng")]
neighbours <- source[,c("lat","lng")]
cat_ids <- source$cat_id
cats_unique <- sort(unique(cat_ids))
test_2 <- get_each_nc(targets,neighbours,cat_ids,cats_unique)
summary(test_2)
