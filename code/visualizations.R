...
# Synthetic data
my_surf <- plot_ly(x=test$x,y=test$y,z=test$z) %>% 
            add_surface()
my_surf
...
# Synthetic data
my_surf <- plot_ly(x=test$x,y=test$y,z=test$z,sizes=c(100,1)) %>% 
  add_markers() %>%
  layout(scene = list(xaxis = list(title = 'x-axis'),
                      yaxis = list(title = 'y-axis'),
                      zaxis = list(title = 'z-axis',range = c(0,10))
                      ))
my_surf
...
# Try it out on the actual data set
my_subset <- subset(dataset,cat_id == 5)
my_plot <- plot_ly(my_subset) %>% 
            add_markers(x = ~ lng,
                        y = ~ lat,
                        z = 0,
                        sizes=c(0.01,0.01)) %>%
            add_surface(x = kd$x, 
                        ...
                        opacity=0.5) %>%
            layout(scene = 
                     list(xaxis = list(title = 'Longitude'),
...
# Plot dataset
library(ggmap)
myMap <- get_map("Taipei City", 
                 zoom = 12,
                 maptype = "roadmap")
sample_index <- sample(1:nrow(dataset),size=1000)
...
myGMap <- ggmap(myMap) + 
          ...
          scale_shape_manual(values = 
                    seq(0,length(unique(dataset$label))))
myGMap


plot_dataset <- ggplot(data = dataset,aes(x=lng,y=lat)) + 
  geom_point(data = dataset,aes(colour = factor(label), shape = factor(label)),size=0.5) +
...
plot_nc_adj <- ggplot() + 
  geom_tile(data = nc_adj,aes(x=lng_bin_ID,y=lat_bin_ID,fill = NC_green),colour = "white") + 
  scale_fill_gradient(low = "white",high = "steelblue")
plot_nc_adj


# ... with Google Map!
ncMap <- get_map("Taipei City", zoom = 12,maptype = "roadmap")
ncGMap <- ggmap(ncMap) + 
  geom_tile(data = nc_list$nc_frame,
            aes(x=lng_bin_mid,y=lat_bin_mid,
                fill = NC_green),alpha=0.5) + 
